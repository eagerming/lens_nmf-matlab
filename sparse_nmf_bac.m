    ...
    if update_h > 0 
        ...
        if params.is_mask
            for i = 1:num_col
                ...
                h_i = h(:,i);
                lambda_i = w * h_i;
                lambda_i(indicate_vec) = 0;
                
                switch div_beta
                    case 1
                        dph = sum(w(indicate_vec, h_ind))' + params.sparsity;
                        dph = max(dph, flr);
                        dmh = w(:, h_ind)' * (v_i ./ lambda_i);
                        h_i(h_ind) = bsxfun(@rdivide, h_i(h_ind, :) .* dmh, dph);
                    case 2
                        dph = w(:, h_ind)' * lambda_i + params.sparsity;
                        dph = max(dph, flr);
                        dmh = w(:, h_ind)' * v_i;
                        h_i(h_ind) = bsxfun(@rdivide, h_i(h_ind, :) .* dmh, dph);
                    otherwise
                        dph = w(:, h_ind)' * lambda_i.^(div_beta - 1) + params.sparsity;
                        dph = max(dph, flr);
                        dmh = w(:, h_ind)' * (v_i .* lambda_i.^(div_beta - 2));
                        h_i(h_ind) = h(h_ind) .* dmh ./ dph;
                end
                h(:, i) = h_i;
            end
        else
    ...
    if update_w > 0 
        if params.is_mask
            for j = 1:num_row
                ...
                w_j = w(j,:);
                lambda_j = w_j * h;
                lambda_j(indicate_vec) = 0;
                
                
                switch div_beta
                    case 1
                        dpw = bsxfun(@plus,sum(h(w_ind, indicate_vec), 2)', ...
                            bsxfun(@times, ...
                            sum((v_j ./ lambda_j) * h(w_ind, :)' .* w(:, w_ind)), w(:, w_ind))); 
                        dpw = max(dpw, flr);
